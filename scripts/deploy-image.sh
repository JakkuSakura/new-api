...
ssh -o BatchMode=yes -o ConnectTimeout=15 -o StrictHostKeyChecking=accept-new "$REMOTE" \
  "set -e; cd '$REMOTE_DIR'; gzip -dc '$REMOTE_ARCHIVE' | docker load; if command -v docker-compose >/dev/null 2>&1; then docker-compose -f '$COMPOSE_FILE' up -d --no-deps new-api; else docker compose -f '$COMPOSE_FILE' up -d --no-deps new-api; fi; rm -f '$REMOTE_ARCHIVE'; docker ps --filter name=^/new-api$ --format '{{.Names}} {{.Status}}'"
