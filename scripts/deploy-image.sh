...
ssh -o BatchMode=yes -o ConnectTimeout=15 -o StrictHostKeyChecking=accept-new "$REMOTE" \
  "set -e; cd '$REMOTE_DIR'; if docker info >/dev/null 2>&1; then DOCKER=docker; elif sudo -n docker info >/dev/null 2>&1; then DOCKER='sudo -n docker'; else echo 'error: remote user cannot access Docker' >&2; exit 1; fi; gzip -dc '$REMOTE_ARCHIVE' | \$DOCKER load; if \$DOCKER image inspect 'localhost/$IMAGE_NAME' >/dev/null 2>&1; then \$DOCKER image tag 'localhost/$IMAGE_NAME' '$IMAGE_NAME'; fi; \$DOCKER rm -f new-api >/dev/null 2>&1 || true; if command -v docker-compose >/dev/null 2>&1; then if [ \"\$DOCKER\" = docker ]; then docker-compose -f '$COMPOSE_FILE' up -d --no-deps new-api; else sudo -n docker-compose -f '$COMPOSE_FILE' up -d --no-deps new-api; fi; else \$DOCKER compose -f '$COMPOSE_FILE' up -d --no-deps new-api; fi; rm -f '$REMOTE_ARCHIVE'; \$DOCKER ps --filter name=^/new-api$ --format '{{.Names}} {{.Status}}'"
